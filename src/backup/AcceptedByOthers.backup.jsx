...
const AcceptedByOthers = () => {
    ...
    const fetchAcceptedRequests = async () => {
      ...
      try {
        ...
        for (const reqDoc of querySnapshot.docs) {
          ...
          const receiverDocs = await getDocs(
            query(collection(db, "employees"), where("userId", "==", requestData.receiverId))
          );
          
          if (!receiverDocs.empty) {
            receiverData = receiverDocs.docs[0].data();
          }
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Requests Accepted by Others</h1>
      ...
      {loading ? (
        ...
        <div className="bg-yellow-50 border border-yellow-100 rounded-md p-4 text-center">
          <p className="text-yellow-700">None of your sent requests have been accepted yet</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="mb-4">
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    ...
                    <p className="font-medium">{request.receiver.currentLocation || "Not Available"}</p>
                  </div>
                  <div>
                    <p className="text-gray-600">Designation:</p>
                    <p className="font-medium">{request.receiver.designation || "Not Available"}</p>
                  </div>
                  <div>
                    <p className="text-gray-600">Department:</p>
                    <p className="font-medium">{request.receiver.department || "Not Available"}</p>
                  </div>
                  <div>
                    <p className="text-gray-600">Grade Level:</p>
                    <p className="font-medium">{request.receiver.gradeLevel || "Not Available"}</p>
                  </div>
                </div>
              ...
              <div className="mt-2 text-right">
                ...
                >
                  ...
                  {selectedRequest === request.id ? "Show Less Details" : "View Contact Details"}
                </button>
              </div>
                ...
                <div className="mt-4 p-3 bg-indigo-50 rounded-md">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div>
                      ...
                      <p className="font-medium">{request.receiver.email || "Not Available"}</p>
                    </div>
                    <div>
                      <p className="text-gray-600">Phone Number:</p>
                      <p className="font-medium">{request.receiver.phoneNumber || "Not Available"}</p>
                    </div>
                  </div>
              ...
              <div className="mt-3 px-3 py-2 bg-green-50 rounded text-sm text-green-700">
                <p>Congratulations! This user has accepted your transfer request. You can now contact them for further proceedings.</p>
              </div>
            </div>
          ))}
        </div>
...
export default AcceptedByOthers; 